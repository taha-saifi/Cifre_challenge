...
| Évaluation critique de la crédibilité | 10 | Note §5 et §16.4, `sources.md` | **Renforcé deux fois** — (a) la section corroboration/contradiction documente une contradiction réelle et une concordance à ne pas confondre avec un écart ; (b) le score d'autorité est maintenant **propagé jusqu'à l'arête** (`source_authority`, joint sur `source_id`, règle unique de `build_source_quality.py`), donc le graphe est triable et filtrable par fiabilité de source — ce n'est plus une limite ouverte. Reste ouvert, et dit comme tel : le score n'est utilisé ni dans les 45 cellules ni dans la sélection de contexte (feuille de route 0–6 mois). Reste moyen sur fraîcheur/stabilité, non instrumentées. |
